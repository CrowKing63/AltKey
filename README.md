...
- **자동완성 전환 후 첫 입력 안정화 (`v1.1.0`)**: 자동완성을 끈 상태에서 물리 키보드처럼 홀드 입력을 사용한 뒤 자동완성을 켜도, 문자·숫자·공백의 첫 입력이 누락되지 않도록 입력 모드 전환 시 진행 중 제스처 상태를 정리합니다.
- **접힌 상태 종료 시 다음 실행 펼침 위치 보존 (`v1.0.9`)**: 키보드를 접어둔 상태(특히 화면 하단에 도킹된 상태)에서 프로그램을 종료하거나 설정을 저장할 때, 다음 실행 시 펼침 상태로 시작되는 특성에 맞추어 펼쳐진 높이 기준의 적절한 상단(Top) 좌표를 계산하고 보존하도록 개선했습니다.
- **닫기 확인 + 최소화 제거**: 닫기 버튼과 `Esc`를 누르면 트레이로 숨길지 먼저 확인하고, 실수로 화면에서 완전히 사라지는 일을 줄이기 위해 상단바 최소화 버튼은 제거했습니다.
- **트레이 숨김 확인 동기화**: 확인 창의 `앞으로 다시 묻지 않기`, 트레이 메뉴의 `닫기 확인 다시 묻지 않기`, 설정의 같은 옵션이 서로 즉시 연동되도록 정리했습니다.
...
- **닫기 전 확인 + 트레이 복귀 경로 안내**: 닫기 버튼과 `Esc`를 눌렀을 때 트레이 숨김을 한 번 더 확인하고, 트레이 아이콘·트레이 메뉴·전역 단축키·앱 재실행으로 복귀하는 방법을 바로 안내
- **트레이 숨김 확인 옵션 즉시 연동**: 확인 창, 트레이 메뉴, 설정 창의 `닫기 확인 다시 묻지 않기` 상태가 같은 설정값으로 즉시 동기화
...
<p align="center">
  <img src="docs/assets/AltKey_BasicPlus_Light.png" alt="AltKey Screenshot" width="800">
  <br>
