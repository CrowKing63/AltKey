...
| `ToggleKoreanSubmode` | 가/A 한영 서브모드 토글 |
...
- **키 반복 입력** — 키를 길게 누르면 물리 키보드처럼 지연 후 반복 입력
- **빈도·빅그램 기반 한국어/영어 자동완성** — 단어 추천 및 다음 단어 예측 (선택적)
...
**`english_label`** / **`english_shift_label`** — QuietEnglish 서브모드에서 표시되는 영문 레이블
...
│   ├── SaveAsDialog.xaml          # 다른 이름으로 저장 대화상자
│   └── UserDictionaryEditorWindow.xaml  # 사용자 사전 편집기
│
├── ViewModels/                    # MVVM 뷰모델 (CommunityToolkit.Mvvm)
├── Models/                        # 데이터 모델 (레이아웃 & 설정 구조체)
├── Services/                      # 핵심 서비스
│   ├── InputService.cs           # SendInput 래퍼, Sticky Keys, 액션 디스패처
│   ├── InputLanguage/             # 언어별 입력 모듈
│   │   ├── KoreanInputModule.cs  # 한국어 조합 입력 엔진
│   │   ├── IInputLanguageModule.cs
│   │   └── InputSubmode.cs       # QuietEnglish 등 서브모드
│   ├── HangulComposer.cs         # 한글 자모 조합기
│   ├── JamoNameResolver.cs       # 자모 이름 해석
...
│   ├── TrayService.cs            # 시스템 트레이 아이콘 관리
│   ├── WindowService.cs          # 창 투명도, 페이딩 관리
│   ├── StartupService.cs          # 시작 프로그램 레지스트리 관리
│   ├── SoundService.cs           # 키 클릭 음향 재생
│   ├── AutoCompleteService.cs    # 단어 자동완성
│   ├── KoreanDictionary.cs       # 한국어 사전
│   ├── EnglishDictionary.cs      # 영어 사전
│   ├── WordFrequencyStore.cs     # 단어 빈도 저장
│   ├── BigramFrequencyStore.cs   # 빅그램 빈도 저장
│   ├── ClipboardService.cs       # 클립보드 히스토리 관리
│   ├── UpdateService.cs         # GitHub Releases 업데이트 확인
│   ├── DownloadService.cs       # 업데이트 다운로드
│   ├── InstallerService.cs      # 업데이트 설치
│   ├── KeyNotationParser.cs      # 키 표기법 파서
│   ├── PathResolver.cs          # 앱 경로 해석
│   └── LiveRegionService.cs     # UI 접근성 알림
├── Controls/                      # 커스텀 컨트롤
│   ├── KeyButton.xaml / .cs      # 키 버튼 (Sticky, Dwell, KeyRepeat)
│   └── NumericAdjuster.xaml / .cs  # 숫자 조절 컨트롤
