...
- **가상 키 홀드 제스처 (`v0.9.5`)**: VirtualKey 모드에서 키를 누른 채 유지하면 물리 키보드처럼 `KeyDown` 상태를 계속 유지하고, 마우스를 떼거나 버튼 밖으로 벗어나면 안전하게 `KeyUp`을 보냅니다.
...
- **VirtualKey 홀드 입력**: 길게 누르면 실제 키를 누르고 있는 상태를 유지하고, 놓거나 버튼 밖으로 벗어나면 즉시 해제
