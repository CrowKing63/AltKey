...
- **Fn 레이어 추가**: 레이아웃 키 하나를 `Fn` 토글로 지정하고, 일반 동작과 Fn 전용 동작을 한 키에 함께 배치할 수 있습니다.
- **행 높이 관리 추가**: 레이아웃 편집기에서 각 행을 `기본` 또는 `낮음` 높이로 맞춰 숫자열·보조열을 더 촘촘하게 구성할 수 있습니다.
- **전체 키 강조 스타일 지원**: 소프트 액센트(살짝 강조) 스타일을 문자 키 포함 모든 키에 적용할 수 있어, 자주 찾는 키를 더 쉽게 구분할 수 있습니다.
- **상단바 커스텀 단축키 편집기**: 상단바에 아이콘형 커스텀 버튼을 추가하고, 툴팁·접근성 이름·실행 액션을 독립 창에서 편집할 수 있습니다.
...
- **상단바 단축키 편집기**: 커스텀 상단바 버튼의 아이콘, 툴팁, 접근성 이름, 실행 액션 편집
