...
### 7.9 기능 문서 동시 갱신
- 기능을 **추가/변경/삭제**한 경우, 같은 작업 안에서 반드시 `Wiki/AltKey.wiki`의 관련 매뉴얼도 함께 갱신한다.
- 위키 갱신 범위는 최소한 다음을 점검한다.
  - 사용자 조작 방법이 바뀌었는가
  - 설정 위치나 안내 문구가 바뀌었는가
  - 관련 페이지의 링크/목차/Home 소개 문구도 함께 바뀌어야 하는가
- 어떤 위키 페이지를 고쳐야 할지 애매하면 먼저 `docs/wiki-meta-manual.md`를 확인해 페이지 책임 범위를 맞춘다.
- 위키를 수정했다면 작업 마무리 전에 `skills/wiki-release.md`를 참고해 `Wiki/AltKey.wiki` 저장소를 별도로 커밋하고 `origin/master`까지 푸시한다.
