...
### 8.4.1 터미널 한글 깨짐 확인 절차
- PowerShell/터미널에서 한글이 깨져 보여도 **파일 자체가 손상되었다고 바로 단정하지 않는다.**
- 먼저 파일 바이트를 확인해 UTF-8 바이트열이 정상인지 본다.
  - 예: `[System.IO.File]::ReadAllBytes(path)`
- 그다음 `Get-Content` 대신 **`.NET UTF-8 읽기`로 다시 확인**한다.
  - 예: `[System.IO.File]::ReadAllText(path)`
  - 또는 `StreamReader(path, [System.Text.Encoding]::UTF8, $true)`
- `.NET UTF-8 읽기`에서는 정상인데 터미널 출력만 깨지면, 그 상태에서는 **출력 결과를 기준으로 본문을 추측 수정하지 않는다.**
- 이런 경우에는 `apply_patch`로 필요한 최소 구간만 수정하고, 수정 전후 샘플 라인을 `.NET UTF-8 읽기` 기준으로 다시 확인한다.
- 한글 `MD`, `C#`, `XAML` 파일의 내용을 확인해야 할 때는 `Get-Content`보다 `.NET UTF-8 읽기`를 우선한다.
