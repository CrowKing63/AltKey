...
- [x] 5.2 별도 도구 적합성 판단
- [x] 5.3 2단계 후보 여부 결정
  - 결과 문서: `docs/altkey-tools/profile-mapping-review.md`

## 6. 메인 앱 진입점 수정

- [x] 6.1 설정창에서 편집기 직접 열기 대신 `AltKey.Tools` 실행으로 전환
- [x] 6.2 특정 도구 직접 열기 지원 여부 결정
  - 결정: 지원함 (`--tool layout`, `--tool dictionary`)
- [x] 6.3 도구 앱 미존재 시 사용자 안내 방식 정의
  - 방식: 경고 대화상자로 `AltKey.Tools.exe` 누락 안내 + 업데이트/파일 위치 확인 문구 제공
