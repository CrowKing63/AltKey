...
5. 사용자는 설정에서 창 크기를 조절할 수 있게 하는데(기존 창조절 기능과 핸들 제거) 백분율 100(4번 값)퍼센트를 기본으로 10퍼센트 단위로 증감할 수 있다. 최종적으로 렌더링 되는 윈도우 사이즈가 이 결과값이다.
...
## 7. 리팩토링 방향 (2026-04-20 결정)

기존 "절대 건드리지 말 것" 리스트는 피드백 루프(`window.Height 직접 조작 → SizeChanged → UpdateKeyUnit → 무한 반복`) 때문에 존재했다. 백분율 기반 스케일링으로 전환하면 루프의 시동점 자체가 사라지므로 더 이상 필요 없다.

**새 아키텍처**: `창 크기 = 기준 크기 × Scale%`. 기준 크기는 레이아웃 메트릭으로부터 산출, Scale만 config에 저장.

### 변경 시 반드시 확인할 것

- 접기 → 펼치기 → 종료 → 재시작 후 키 크기가 유지되는지
- 바 ON/OFF 전환 후 종료 → 재시작 시 키 크기가 유지되는지
- Scale 변경 후 키 비율이 유지되는지 (왜곡 없는지)
- 사용자 커스텀 레이아웃에서도 기준 크기가 올바르게 계산되는지
