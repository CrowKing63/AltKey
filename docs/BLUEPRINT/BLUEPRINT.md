...
| **UI 라이브러리** | 기본 WPF 컨트롤 + 커스텀 스타일 | 입력 안정성과 접근성 중심으로 외형을 직접 제어 |
...
| **과한 배경 효과는 가독성 저하 가능** | 장식용 배경 효과가 입력면 대비를 흐릴 수 있음 | 장식 효과보다 명확한 대비와 투명도 제어를 우선 |
...
### 4.4 배경 투명도와 가독성
- 장식용 효과 대신 입력면 가독성을 해치지 않는 반투명 배경 사용
- 유휴 상태에서만 투명도를 낮추고, 사용 중에는 충분한 대비 유지
- 시각 효과보다 접근성과 상태 인지를 우선
...
| **1** | 핵심 윈도우 관리 | NoActivate + TopMost + 드래그 + 투명도 제어 |
...
- [WPF 공식 문서](https://learn.microsoft.com/en-us/dotnet/desktop/wpf/) — 기본 윈도우, 리소스, 스타일 구조
