...
- **Bigram/문맥 제안**: 2026-04-18 릴리스에서 구현 완료 — `docs/ac-bigram/` 참조. `KoreanInputModule`이 마지막 확정 단어를 추적하여 `KoreanDictionary.GetSuggestions(prefix, prevWord)` 경로로 문맥 가산을 적용한다.
